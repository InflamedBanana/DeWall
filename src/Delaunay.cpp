...
namespace Delaunay
{
    ...
    namespace
    {
        ...
        void MakeSimplex(Edge &_edge, const PointSet &_pointSet, Triangle *&_triangle, const vector<Vect3<float>> *_convexHull)
        {
            ...
            bool cSide = Dot(_pointSet[_edge.oppositePoint] - halfPlane.position, halfPlane.position + halfPlane.normal) >= .0f;
            ...
            {
                bool halfPlanePos = Dot(_pointSet[i] - halfPlane.position, halfPlane.position + halfPlane.normal) >= .0f;

                if (i == _edge.a || i == _edge.b || _edge.oppositePoint == i ||
                    cSide && halfPlanePos ||
                    !cSide && !halfPlanePos) // check if point is in HalfSpace.
                    continue;

                float radius = 
                    GetCircumCircleRadius(sqrt(distAB), 
            ...
            _triangle = new Triangle(&_edge, new Edge(_edge.a, pointIndex, _edge.b), new Edge(_edge.b, pointIndex, _edge.a));
        }
        ...
        {
            if (Math3D::RayPlaneIntersection(_vertices[_edge.a], _vertices[_edge.b] - _vertices[_edge.a], _wall.position, _wall.normal))
            {
